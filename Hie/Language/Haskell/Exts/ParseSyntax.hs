...
module Hie.Language.Haskell.Exts.ParseSyntax where
...
instance Annotated PExp where
    ...
    amap f e = case e of
        ...
        Do l ss                 -> Do (f l) ss
        MDo l ss                -> MDo (f l) ss
        TupleSection l b mes      -> TupleSection (f l) b mes
...
instance Functor PExp where
      fmap f e = case e of
          ...
          TupleSection l b mes      -> TupleSection (f l) b (map (fmap (fmap f)) mes)
...
instance Functor PType where
    fmap f t = case t of
      ...
      TySplice  l s                 -> TySplice (f l) (fmap f s) 
