...
CHRIS:
>       | '$(' trueexp ')'            { TySplice  ($1 <^^> $3 <** [$1,$3]) $ ParenSplice (ann $2) $2 }
...
>       : ivar                          { IPVar (ann $1) $1 }
>       | qvar                          { Var (ann $1) $1 }
>       |  'family'                     { Var (nIS $1) (UnQual (nIS $1) (Ident (nIS $1) "family"))  }
>       | gcon                          { $1 }
>       | literal                       { Lit (ann $1) $1 }
>       | '(' texp ')'                  { Paren ($1 <^^> $3 <** [$1,$3]) $2 }

CHRIS: this is wrong, but doesn't matter
>       | '(#' texp '#)'                  { Paren ($1 <^^> $3 <** [$1,$3]) $2 }
CHRIS: allow (#)
>       | '(#' ')'    { Var (nIS $1) (UnQual (nIS $1) (Symbol (nIS $1) "#"))  }
...
CHRIS: Here's the same code, but for hashes:

>       | '(#' texp tsectend2            { TupleSection ($1 <^^> head (snd $3) <** $1:reverse (snd $3)) Unboxed (Just $2 : fst $3) }
>       | '(#' commas texp '#)'           { TupleSection ($1 <^^> $4 <** $1:reverse ($4:$2)) Unboxed
>                                                       (replicate (length $2) Nothing ++ [Just $3]) }
>       | '(#' commas texp tsectend2      { TupleSection ($1 <^^> head (snd $4) <** $1:reverse (snd $4 ++ $2)) Unboxed
>                                                       (replicate (length $2) Nothing ++ Just $3 : fst $4) }
...
> tsectend2 :: { ([Maybe (PExp L)],[S]) }
>       : commas texp tsectend2          { let (mes, ss) = $3 in (replicate (length $1 - 1) Nothing ++ Just $2 : mes, ss ++ $1) }
>       | commas texp '#)'               { (replicate (length $1 - 1) Nothing ++ [Just $2], $3 : $1) }
>       | commas '#)'                    { (replicate (length $1) Nothing, $2 : $1) }
